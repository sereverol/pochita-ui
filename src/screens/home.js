...
const Home = ({ navigation, route }) => {
        ...
        <View style={{ flexDirection: 'row' }}>
          ...
          >
            <View style={{ flexDirection: 'row' }}>
              ...
              <Text style={{ fontSize: 16, color: '#a4a4a4' }}>{stamp}</Text>
            </View>
          </TouchableOpacity>
        </View>
  ...
  const deleteList = async (listItem) => {
    const data = await Http.send('DELETE', `/api/list/${listItem.id}`, null);
  ...
  const updateList = async () => {
    if (!Field.checkFields([newList.title])) {
      ...
      const data = await Http.send('PUT', '/api/list/', jsonAux);
  ...
  const getList = async () => {
    ...
    const data = await Http.send('GET', `/api/list/user/${id}`, null);
    let res = [];
  ...
  const submitNewList = async () => {
    if (!Field.checkFields([newList.title])) {
      ...
      setLoading(true);
      const id = route.params.id;
      const data = await Http.send('POST', '/api/list/', {
        ...newList,
        userId: id,
      });
  ...
  return (
    <View style={homeStyles.container}>
      ...
      >
        <View style={homeStyles.centeredView}>
          <View style={homeStyles.modalView}>
            ...
            <TextInput
              placeholder="Write the list's title"
              style={{
                marginBottom: 15,
              ...
              onSubmitEditing={() => {
                modal.type == 'create' ? submitNewList() : updateList();
              }}
            />
            <TouchableOpacity
      ...
      {/* <View style={homeStyles.viewTitle}>
        <Text style={homeStyles.textTitle}>Welcome, {route.params.name}</Text>
      </View> */}
      <Header>Pochita!</Header>
      ...
      {/* <View style={homeStyles.viewSecondary}>
        <TouchableOpacity
          onPress={() =>
            Alert.alert('Sorry!', 'This area is under maintenance')
          }
          style={homeStyles.secondary}
        >
          <Text style={homeStyles.textSecondary}>Priority</Text>
      ...
      <View style={homeStyles.viewSecondary}>
        <TouchableOpacity
          ...
          style={homeStyles.secondary}
        >
          <Text style={homeStyles.textSecondary}>Stats</Text>
          <Icon name="podium-outline" color="gray" type="ionicon" size={30} />
        </TouchableOpacity>
      </View> */}
    </View>
  );
