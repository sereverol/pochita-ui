...
const TaskDetail = ({ route }) => {
  ...
  const alertForDelete = (item, type) => {
    Alert.alert(
      'Delete',
      `Are you sure delete ${
        type == 'step' ? item.description : item.data.title
      }`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'OK',
          onPress: () => {
            type == 'step' ? deleteStep(item) : deleteArchives(item);
          },
        },
      ],
      { cancelable: false }
  ...
  const openImagePickerAsync = async () => {
    ...
    if (permission.status != 'granted') {
      ...
      if (imgResult.cancelled == true) {
        ...
        imgResult = { ...imgResult, title: aux[0], format: aux[1] };
  ...
  const updateField = async (type, field) => {
    ...
    const data = await Http.send('PUT', '/api/task/field', jsonAux);
  ...
  const sendEditStep = async (type) => {
    const data = await Http.send('PUT', '/api/step/', { ...newStep, type });
  ...
  const addStep = async () => {
    if (!Field.checkFields([newStep.description])) {
      ...
      const data = await Http.send('POST', '/api/step/', {
        ...newStep,
        taskId: route.params.item.id,
      });
  ...
  const deleteStep = async (stepItem) => {
    const data = await Http.send('DELETE', `/api/step/${stepItem.id}`, null);
  ...
  const addArchive = async (archive) => {
    if (
      ...
      const data = await Http.send('POST', '/api/archive/:taskId', {
        data: archive.data,
        taskId: route.params.item.id,
      });
  ...
  const deleteArchives = async (archiveItem) => {
    ...
    if (!data) {
      ...
      switch (data.typeResponse) {
        case 'Success':
          toast(data.message);
          const archivesAux = task.archives.filter(
            (i) => i.id != archiveItem.id
          );
          const taskAux = { ...task, archives: archivesAux };
        ...
        {data1 != null ? (
          <View>
            <Text style={{ color: 'gray' }}>
              {data1.toString().split(' ').splice(1, 3).join('-')}
            </Text>
            <Text style={{ color: 'gray' }}>
              {data1.toString().split(' ')[4]}
            </Text>
          </View>
        ) : (
          <View>
    ...
    <Card>
      {task.archives.map((item) => (
        ...
        >
          <View style={{ borderRadius: 10, backgroundColor: 'grey' }}>
            <Avatar size="small" title={item.data.format} activeOpacity={0.7} />
          </View>
      ...
      >
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          <Icon name="attach-outline" color="gray" type="ionicon" size={20} />
          <Text>Add archives..</Text>
        </View>
        <Icon
  ...
  const CardDatesC = () => (
    <Card>
      ...
      >
        ...
        <TextC2 data1={dateExp.data} data2={route.params.item.dateExpiration} />
        <Icon
          name="chevron-forward-outline"
          color="gray"
          type="ionicon"
  ...
  const CardNoteC = () => (
    <Card>
      {!note.flag ? null : (
        ...
        >
          <Text style={{ color: 'white' }}>Finish editing</Text>
        </TouchableOpacity>
      )}
      <TextInput
  ...
  return (
    <View style={{ paddingTop: 24, flex: 1 }}>
      ...
      >
        <CheckBox checked={task.check} onPress={handleCheck} />
        <Text style={{ color: 'gray', fontSize: 30 }}>
          {route.params.item.title}
        </Text>
        <CheckBox
          checkedIcon={
            <Icon name="star" color="gold" type="ionicon" size={30} />
          }
          uncheckedIcon={
            <Icon name="star-outline" color="grey" type="ionicon" size={30} />
          }
