...
const ListDetail = ({ navigation, route }) => {
  ...
  const renderUnderlayLeft = ({ item, percentOpen, close }) => (
    <Animated.View
      style={[
        listDetailStyles.backRow,
        listDetailStyles.underlayLeft,
        { opacity: percentOpen },
      ]}
    >
      <Icon
  ...
  const renderItem = ({ item, index, drag }) => (
    ...
    >
      <View style={listDetailStyles.item}>
        <CheckBox checked={item.check} onPress={() => handleCheck(item)} />
        <View style={listDetailStyles.row}>
          ...
          <CheckBox
            checkedIcon={
              <Icon name="star" color="gold" type="ionicon" size={20} />
            }
            uncheckedIcon={
              <Icon name="star-outline" color="grey" type="ionicon" size={20} />
            }
  ...
  const updateField = async (type, field, id) => {
    const data = await Http.send('PUT', '/api/task/field', { type, id, field });
  ...
  const deleteTask = async (taskItem) => {
    const data = await Http.send('DELETE', `/api/task/${taskItem.id}`, null);
  ...
  const submitNewTask = async () => {
    if (!Field.checkFields([newTask.title])) {
      ...
      const data = await Http.send('POST', '/api/task/', {
        ...newTask,
        listId: id,
      });
  ...
  return (
    <View style={listDetailStyles.container}>
        ...
        <View style={listDetailStyles.centeredView}>
          <View style={listDetailStyles.modalView}>
            ...
            <TextInput
              ...
              autoFocus
              onChangeText={(title) => setNewTask({ ...newTask, title: title })}
              onSubmitEditing={() => {
                modal.type == 'create' ? submitNewTask() : updateTasktitle();
              }}
            />
            <TouchableOpacity
              onPress={() => {
      ...
      <Header>{route.params.item.title}</Header>
